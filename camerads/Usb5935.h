...
const long USB5935_INPUT_N2500_P2500mV	= 0x02; // ±25000mV
...
extern "C" {
    ...
    HANDLE DEVAPI FAR PASCAL USB5935_CreateDeviceEx(int DevicePhysID = 0); // 创建设备对象(该函数使用板上物理ID,由拔码开关DID1实现)
    ...
    BOOL DEVAPI FAR PASCAL USB5935_ReleaseDevice(HANDLE hDevice);    // 设备句柄
	...
	ULONG DEVAPI FAR PASCAL USB5935_GetFileLength(HANDLE hFileObject); // 取得指定文件长度（字节）
	...
	//############################ 线程操作函数 ################################
	DEVAPI HANDLE FAR PASCAL USB5935_CreateSystemEvent(void); 	// 创建内核系统事件对象
	...
	//################# 其他附加函数 ########################
	BOOL DEVAPI FAR PASCAL USB5935_GetAppPath(LPCTSTR strAppPath, LONG PathSizeBytes); // 取得产品应用程序的安装路径
	...
	BOOL DEVAPI FAR PASCAL USB5935_SaveParaInt(HANDLE hDevice, LPCTSTR strParaName, int nValue); // 将整型数据保存到注册表中(Device-x\Others)
	UINT DEVAPI FAR PASCAL USB5935_LoadParaInt(HANDLE hDevice, LPCTSTR strParaName, int nDefaultVal); // 将整型数据从注册表中回读出来(Device-x\Others)
	BOOL DEVAPI FAR PASCAL USB5935_SaveParaString(HANDLE hDevice, LPCTSTR strParaName, LPCTSTR strParaVal); // 将字符串数据保存到注册表中(Device-x\Others)
	BOOL DEVAPI FAR PASCAL USB5935_LoadParaString(HANDLE hDevice, LPCTSTR strParaName, LPCTSTR strParaVal, LPCTSTR strDefaultVal); // 将字符串数据从注册表中回读出来(Device-x\Others)

 	DWORD DEVAPI FAR PASCAL USB5935_GetLastErrorEx(LPCTSTR strFuncName, LPCTSTR strErrorMsg);
...
// 自动包含驱动函数导入库
	...
	#pragma message("======== Automatically linking with USB5935.dll...")
	#pragma message("======== Successfully linked with USB5935.dll")
#endif
